...
public class ReimbursementDAO implements ReimbursementDAOI{
	...
	public List<Reimbursement> getReimbursementsByUser(int id){
		...
		try(Connection conn = ConnectionUtil.getConnection()){
			PreparedStatement ps = conn.prepareStatement("SELECT * FROM ers.ers_reimbursement WHERE reimb_author = ? ORDER BY reimb_id");
			ps.setInt(1, id);
	...
	public Reimbursement getReimbursementByID(int id) {
		...
		try(Connection conn = ConnectionUtil.getConnection()){
			PreparedStatement ps = conn.prepareStatement("SELECT * FROM ers.ers_reimbursement WHERE reimb_id = ?");
			ps.setInt(1, id);
			ResultSet rs = ps.executeQuery();
	...
	public List<Reimbursement> retrieveAllReimbursements() {
		...
		try(Connection conn = ConnectionUtil.getConnection()){
			PreparedStatement ps = conn.prepareStatement("SELECT * FROM ers.ers_reimbursement ORDER BY reimb_id");
			ResultSet rs = ps.executeQuery();
	...
	public int addReimbursement(Reimbursement rb) {
		...
		try(Connection conn = ConnectionUtil.getConnection()){
			PreparedStatement ps = conn.prepareStatement("INSERT INTO ers.ers_reimbursement (reimb_amount, reimb_submitted, reimb_resolved, reimb_description,reimb_author,reimb_status_id,reimb_type_id)"
	...
	public boolean approveDeny(String choice, int id, int resolverId) {
		...
		try(Connection conn = ConnectionUtil.getConnection()) {
			PreparedStatement ps = conn.prepareStatement("UPDATE ers.ers_reimbursement SET reimb_status_id = ?, reimb_resolved = ?, reimb_resolver = ? WHERE reimb_id = ? AND reimb_author != " + resolverId);
			ps.setInt(4, id);
